...
graph LR
    subgraph Color Legend
        ...
        s3(S3)
        iot_core(IoT Core)
        event_bridge(EventBridge Scheduler)
        lambda(Lambda)
...
graph LR
    subgraph AWS
        ...
        s3_lambda --> |scheduled<br>updates| event_bridge(EventBridge Scheduler)
        event_bridge --> |triggered<br>by timer| scheduled_lambda(EventBridge &lambda;)
        scheduled_lambda --> |scheduled<br>jobs| iot_core_jobs
        event_bridge --> |failed<br>events| dlq(Dead Letter Queue)
    end
...
sequenceDiagram
    ...
    participant event_bridge as EventBridge<br>Scheduler
    participant eb_lambda as EventBridge<br>Lambda
    ...
    s3_lambda->>event_bridge: create<br>scheduler updates
    event_bridge->>eb_lambda: update available<br>triggered by event timer
    eb_lambda->>iot_core_jobs: create jobs<br>for scheduled update
    iot_core_jobs->>device_firmware: update device-firmware version
