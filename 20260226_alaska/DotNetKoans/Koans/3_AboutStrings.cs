...
broken line";
	...
	{
		var str = string.Format(CultureInfo.InvariantCulture, "{0:t}", DateTime.Parse("12/16/2011 2:35:02 PM", CultureInfo.InvariantCulture));
    ...
    // or "P 35" if your culture uses t as the AM/PM designator — this is actually culture-dependent too)
    // Be careful: single-letter format specifiers in DateTime can be ambiguous. When they appear alone, t means "short time pattern" (like 2:35 PM).
    // But inside a custom format string (which this is, because it has multiple characters), t means "first character of the AM/PM designator". So:
    // Context	t means	Example
    // Standard format: {0:t}	Short time pattern	2:35 PM
    // Custom format: {0:t m}	First char of AM/PM	P 35
